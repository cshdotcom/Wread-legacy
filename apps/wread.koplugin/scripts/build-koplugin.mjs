#!/usr/bin/env node
/**
 * Build a WRead-${version}-1.koplugin.zip locally for sideloading
...
Default out:     ./WRead-<version>-1.koplugin.zip`);
      process.exit(0);
    } else {
...
function main() {
  ...
  const out = path.resolve(args.out || `WRead-${version}-1.koplugin.zip`);
