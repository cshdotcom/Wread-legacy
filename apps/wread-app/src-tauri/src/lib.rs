...
pub fn run() {
    ...
    let builder = builder.plugin(
        tauri_plugin_single_instance::Builder::new()
            ...
            .dbus_id("com.bilingify.wread".to_owned())
            .build(),
    );
    ...
    // See https://github.com/wread/wread/issues/4398.
    #[cfg(desktop)]
    ...
    builder
        .setup(|#[allow(unused_variables)] app| {
            ...
            let updater_disabled = {
                ...
                std::env::var("WREAD_DISABLE_UPDATER").is_ok() || is_flatpak
            };
            #[cfg(not(desktop))]
            ...
            let init_script = format!(
                r#"
                    if ({is_eink}) window.__WREAD_IS_EINK = true;
                    if ({cli_access}) window.__WREAD_CLI_ACCESS = true;
                    if ({is_appimage}) window.__WREAD_IS_APPIMAGE = true;
                    if ({updater_disabled}) window.__WREAD_UPDATER_DISABLED = true;
