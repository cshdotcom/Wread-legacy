; WRead NSIS Installer Hooks
; Registers/unregisters the thumbnail provider DLL for Windows Explorer thumbnails

!include "LogicLib.nsh"

; CLSID for WRead Thumbnail Provider
!define CLSID_WREAD_THUMBNAIL "{A1B2C3D4-E5F6-7890-ABCD-EF1234567890}"
...
!macro NSIS_HOOK_POSTINSTALL
    DetailPrint "Registering WRead Thumbnail Provider..."
    ...
    WriteRegStr HKCR "CLSID\${CLSID_WREAD_THUMBNAIL}" "" "WRead Thumbnail Provider"
    WriteRegStr HKCR "CLSID\${CLSID_WREAD_THUMBNAIL}\InprocServer32" "" "$INSTDIR\readest_thumbnail.dll"
    WriteRegStr HKCR "CLSID\${CLSID_WREAD_THUMBNAIL}\InprocServer32" "ThreadingModel" "Apartment"
    ...
    WriteRegDWORD HKCR "CLSID\${CLSID_WREAD_THUMBNAIL}" "DisableProcessIsolation" 1
    
    ; ========== EPUB ==========
    ; Register thumbnail handler directly on extension (this is what Windows Shell uses)
    WriteRegStr HKCR ".epub\ShellEx\${SHELL_THUMBNAIL_HANDLER}" "" "${CLSID_WREAD_THUMBNAIL}"
    
    ; ========== MOBI ==========
    WriteRegStr HKCR ".mobi\ShellEx\${SHELL_THUMBNAIL_HANDLER}" "" "${CLSID_WREAD_THUMBNAIL}"
    
    ; ========== AZW ==========
    WriteRegStr HKCR ".azw\ShellEx\${SHELL_THUMBNAIL_HANDLER}" "" "${CLSID_WREAD_THUMBNAIL}"
    
    ; ========== AZW3 ==========
    WriteRegStr HKCR ".azw3\ShellEx\${SHELL_THUMBNAIL_HANDLER}" "" "${CLSID_WREAD_THUMBNAIL}"
    
    ; ========== KF8 ==========
    WriteRegStr HKCR ".kf8\ShellEx\${SHELL_THUMBNAIL_HANDLER}" "" "${CLSID_WREAD_THUMBNAIL}"
    
    ; ========== FB2 ==========
    WriteRegStr HKCR ".fb2\ShellEx\${SHELL_THUMBNAIL_HANDLER}" "" "${CLSID_WREAD_THUMBNAIL}"
    
    ; ========== CBZ ==========
    WriteRegStr HKCR ".cbz\ShellEx\${SHELL_THUMBNAIL_HANDLER}" "" "${CLSID_WREAD_THUMBNAIL}"
    
    ; ========== CBR ==========
    WriteRegStr HKCR ".cbr\ShellEx\${SHELL_THUMBNAIL_HANDLER}" "" "${CLSID_WREAD_THUMBNAIL}"
...
!macro NSIS_HOOK_PREUNINSTALL
    DetailPrint "Unregistering WRead Thumbnail Provider..."
    
    ; Remove CLSID
    DeleteRegKey HKCR "CLSID\${CLSID_WREAD_THUMBNAIL}"
