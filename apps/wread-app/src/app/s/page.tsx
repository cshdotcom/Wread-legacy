...
import { WREAD_WEB_BASE_URL, SHARE_BASE_URL } from '@/services/constants';
...
// wread://share/{token} deep link before /s ever loads.
...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  if (!result.ok) {
    return {
      title: 'Share link unavailable · WRead',
      description: 'This share link is no longer available.',
    };
  }
  const { share } = result;
  const shareUrl = `${SHARE_BASE_URL}/${token}`;
  const ogImage = `${WREAD_WEB_BASE_URL}/api/share/${token}/og.png`;

  return {
    title: `${share.bookTitle} · Shared via WRead`,
    description: share.bookAuthor
      ? `${share.bookAuthor} · Shared via WRead`
      : 'Shared via WRead',
    openGraph: {
      type: 'book',
      url: shareUrl,
