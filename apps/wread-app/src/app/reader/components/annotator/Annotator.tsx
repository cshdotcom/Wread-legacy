...
const Annotator: React.FC<{ bookKey: string; contentInsets: Insets }> = ({
  ...
  // WRead: Dictionary and translation features require login
  const { user } = useAuth();
  ...
  const handleDictionary = () => {
    if (!isLoggedIn) return; // WRead: Dictionary requires login
    if (!selection || !selection.text) return;
  ...
  const handleTranslation = () => {
    if (!isLoggedIn) return; // WRead: Translation requires login
    if (!selection || !selection.text) return;
    setShowAnnotPopup(false);
    setShowDeepLPopup(true);
  };
  ...
  const buildToolButton = (type: AnnotationToolType) => {
    ...
    switch (type) {
      ...
      case 'dictionary':
        return { tooltipText: _(label), Icon, onClick: handleDictionary, disabled: !isLoggedIn };
      case 'translate':
        return { tooltipText: _(label), Icon, onClick: handleTranslation, disabled: !isLoggedIn };
      case 'tts':
