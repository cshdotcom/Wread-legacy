...
export interface ImportFromFolderResult {
   ...
   * — WRead will read each file straight from its original location
   * instead of copying it into Books/<hash>/. Sidecars (cover, config,
   * notes) still live in WRead's data dir, so deleting the local
   * copy of a book on WRead's side will physically remove the
   * source file under the registered folder. Defaults to `false`,
   * which keeps the legacy "copy into WRead" behaviour and leaves
