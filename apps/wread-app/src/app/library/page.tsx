 ...
 */
const LAST_IMPORT_FOLDER_KEY = 'wread:lastImportFolder';
/**
 ...
 */
const LAST_IMPORT_FOLDER_MODE_KEY = 'wread:lastImportFolderMode';
/**
 ...
 */
const LAST_IMPORT_FOLDER_FORMATS_KEY = 'wread:lastImportFolderFormats';
/**
 * Key used to persist the last "File size larger than" threshold (KB).
 * Stored as a stringified non-negative integer.
 */
const LAST_IMPORT_FOLDER_MIN_SIZE_KEY = 'wread:lastImportFolderMinSizeKB';
/**
 ...
 */
const LAST_IMPORT_FOLDER_READ_IN_PLACE_KEY = 'wread:lastImportFolderReadInPlace';
...
const LibraryPageContent = ({ searchParams }: { searchParams: ReadonlyURLSearchParams | null }) => {
  ...
  // https://github.com/wread/wread/issues/3782.
    ...
    async (appService: AppService, openWithFiles: string[], libraryBooks: Book[]) => {
      ...
      for (const file of openWithFiles) {
        ...
        try {
          ...
          // A file shared into WRead on mobile (the OS share-sheet) is a
          // "Send to WRead" capture — force it to the cloud so it syncs to
  ...
  const importBooks = async (files: SelectedFile[], groupId?: string) => {
    ...
    // WRead's own Books/ prefix is resolved once at app init and persisted
  ...
  const validatePickedDirectory = (path: string): boolean => {
    ...
    if (isOnMyIPhoneRoot) {
      eventDispatcher.dispatch('toast', {
        ...
        timeout: 6000,
        message: _(
          'iOS doesn\'t allow importing the "On My iPhone" root. Open it and pick a specific subfolder (e.g. WRead, Downloads), then try again.',
        ),
      });
      return false;
    }
