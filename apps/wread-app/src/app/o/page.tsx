...
import { DOWNLOAD_WREAD_URL, WREAD_WEB_BASE_URL } from '@/services/constants';
...
const ANDROID_PACKAGE = 'com.bilingify.wread';
const FALLBACK_TIMEOUT_MS = 1500;
const DESKTOP_FALLBACK_DELAY_MS = 1000;

const buildIntentUrl = (path: string, fallbackUrl: string) => {
  const cleanPath = path.replace(/^\//, '');
  return `intent://${cleanPath}#Intent;scheme=wread;package=${ANDROID_PACKAGE};S.browser_fallback_url=${encodeURIComponent(fallbackUrl)};end`;
};
...
const OpenAnnotationLanding = () => {
  ...
  useEffect(() => {
    ...
    if (platform === 'android-chromium') {
      const absoluteFallback = `${WREAD_WEB_BASE_URL}${webReaderUrl}`;
      ...
      <main className='bg-base-200 flex min-h-dvh flex-col items-center justify-center p-4 sm:p-8'>
        <Card>
          <div className='flex flex-col items-center text-center'>
            ...
            <a href='https://wread.com' className='btn btn-ghost btn-block mt-6' rel='noopener'>
              {_('Go to WRead')}
            </a>
          </div>
        </Card>
  ...
  return (
    <main className='bg-base-200 flex min-h-dvh flex-col items-center justify-center p-4 sm:p-8'>
      <Card>
        <BrandHeader
          title={_('Open in WRead')}
          subtitle={
            showManualOpen
              ? _("If WRead didn't open automatically, choose an option below:")
        ...
        {!showManualOpen && (
          ...
          >
            ...
            <span className='text-base-content/70 text-sm'>{_('Opening WRead...')}</span>
          </div>
        )}
