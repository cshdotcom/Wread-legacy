...
import { WREAD_BUILTIN_DICTIONARIES } from '@/services/dictionaries/builtinDicts';
...
const BUILTIN_WEB_ORDER = [
  BUILTIN_WEB_SEARCH_IDS.juheSearch,
...
// WRead: Include built-in MDX dictionary IDs in default provider order
const WREAD_BUILTIN_DICT_IDS = WREAD_BUILTIN_DICTIONARIES.map((d) => d.id);
...
const DEFAULT_DICTIONARY_SETTINGS: DictionarySettings = {
  ...
  providerEnabled: {
    ...
    // WRead: Enable all built-in MDX dictionaries by default
    ...Object.fromEntries(WREAD_BUILTIN_DICT_IDS.map((id) => [id, true])),
  },
  webSearches: [],
};
