...
import { WREAD_UPDATER_FILE, WREAD_CHANGELOG_FILE } from '@/services/constants';
...
export const UpdaterContent = ({
  ...
  useEffect(() => {
    ...
    const checkAndroidUpdate = async () => {
      ...
      const response = await fetch(WREAD_UPDATER_FILE);
      ...
      if (semver.gt(data.version, currentVersion)) {
        ...
        const apkFilePath = await appService.resolveFilePath(
          `WRead_${data.version}_${arch}.apk`,
          'Cache',
        );
        setUpdate({
          currentVersion,
    ...
    const checkWindowsPortableUpdate = async () => {
      ...
      if (semver.gt(data.version, currentVersion)) {
        ...
        const exeFileName = `WRead_${data.version}_${arch}-portable.exe`;
        ...
        setUpdate({
          ...
          downloadAndInstall: async (onEvent) => {
            ...
            try {
              ...
              const command = Command.create('start-wread', ['/C', 'start', '', exeFilePath]);
              await command.spawn();
    ...
    const checkAppImageUpdate = async () => {
      ...
      if (semver.gt(data.version, currentVersion)) {
        ...
        const downloadUrl = data.platforms[platformKey]?.url as string;
        const appImageFileName = `WRead_${data.version}_${arch}.AppImage`;
  ...
  useEffect(() => {
    const fetchChangelogs = async (fromVersion: string): Promise<Changelog[]> => {
      try {
        const fetch = isTauriAppPlatform() ? tauriFetch : window.fetch;
        const res = await fetch(WREAD_CHANGELOG_FILE);
        // Gitea API returns an array of releases
        const data = await res.json();
        const releases: Array<{ tag_name?: string; version?: string; created_at?: string; date?: string; body?: string; notes?: string[] }> =
          Array.isArray(data) ? data : [];

        const cleanVer = (v: string) => (v.startsWith('v') ? v.slice(1) : v);

        let entries = releases
          .map((r) => ({
            version: cleanVer(r.tag_name || r.version || ''),
            date: r.created_at || r.date || '',
            notes: r.notes || (r.body ? r.body.split(/\n/).filter((l: string) => l.trim()) : []),
          }))
          .filter((r) => r.version && semver.gt(r.version, fromVersion))
          .sort((a, b) => semver.rcompare(a.version, b.version));
        entries = entries.length ? entries : releases.slice(0, 3).map((r) => ({
          version: cleanVer(r.tag_name || r.version || ''),
          date: (r.created_at || r.date || ''),
          notes: r.notes || (r.body ? r.body.split(/\n/).filter((l: string) => l.trim()) : []),
        }));
        return entries.map((info) => ({
          ...
          date: info.date ? new Date(info.date).toLocaleDateString() : '',
          notes: info.notes,
        }));
      } catch (error) {
          ...
          {checkUpdate ? (
            <div className='text-base-content flex-grow text-sm'>
              <h2 className='mb-4 text-center font-bold sm:text-start'>
                {_('A new version of WRead is available!')}
              </h2>
              <p className='mb-2'>
                {_('WRead {{newVersion}} is available (installed version {{currentVersion}}).', {
                  newVersion,
                  currentVersion,
                })}
              </p>
                ...
                {changelogs.length > 0 && semver.gt(changelogs[0]!.version, currentVersion) ? (
                  <div className='flex gap-2'>
                    ...
                    {appService?.isAndroidApp && (
                      <Link
                        href='https://play.google.com/store/apps/details?id=com.bilingify.wread'
                        target='_blank'
                        rel='noopener noreferrer'
                        className='btn btn-primary btn-sm'
                      >
...
export const UpdaterWindow = () => {
  ...
  return (
    <Dialog
      id='updater_window'
      isOpen={isOpen}
      title={checkUpdate ? _('Software Update') : _("What's New in WRead")}
