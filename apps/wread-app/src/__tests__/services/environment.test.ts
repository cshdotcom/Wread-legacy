...
vi.mock('@/services/constants', () => ({
  WREAD_WEB_BASE_URL: 'https://web.wread.com',
  WREAD_NODE_BASE_URL: 'https://node.wread.com',
}));
...
beforeEach(() => {
  ...
  delete (window as unknown as Record<string, unknown>)['__WREAD_CLI_ACCESS'];
  delete (window as unknown as Record<string, unknown>)['__WREAD_RUNTIME_CONFIG'];
});
...
describe('environment', () => {
  ...
  // ── hasCli ─────────────────────────────────────────────────────
  describe('hasCli', () => {
    test('returns true when __WREAD_CLI_ACCESS is true', async () => {
      window.__WREAD_CLI_ACCESS = true;
      const { hasCli } = await import('@/services/environment');
      expect(hasCli()).toBe(true);
    });

    test('returns false when __WREAD_CLI_ACCESS is not set', async () => {
      const { hasCli } = await import('@/services/environment');
      expect(hasCli()).toBe(false);
    });

    test('returns false when __WREAD_CLI_ACCESS is explicitly false', async () => {
      window.__WREAD_CLI_ACCESS = false;
      const { hasCli } = await import('@/services/environment');
      expect(hasCli()).toBe(false);
    });
  });
  ...
  describe('getBaseUrl', () => {
    ...
    test('falls back to WREAD_WEB_BASE_URL when env var not set', async () => {
      delete env['NEXT_PUBLIC_API_BASE_URL'];
      const { getBaseUrl } = await import('@/services/environment');
      expect(getBaseUrl()).toBe('https://web.wread.com');
    });
  });
  ...
  describe('getNodeBaseUrl', () => {
    ...
    test('falls back to WREAD_NODE_BASE_URL when env var not set', async () => {
      delete env['NEXT_PUBLIC_NODE_BASE_URL'];
      const { getNodeBaseUrl } = await import('@/services/environment');
      expect(getNodeBaseUrl()).toBe('https://node.wread.com');
    });
  });
  ...
  describe('getAPIBaseUrl', () => {
    ...
    test('returns full URL in production', async () => {
      ...
      delete env['NEXT_PUBLIC_API_BASE_URL'];
      const { getAPIBaseUrl } = await import('@/services/environment');
      expect(getAPIBaseUrl()).toBe('https://web.wread.com/api');
    });
  ...
  describe('getNodeAPIBaseUrl', () => {
    ...
    test('returns full node URL in production', async () => {
      ...
      delete env['NEXT_PUBLIC_NODE_BASE_URL'];
      const { getNodeAPIBaseUrl } = await import('@/services/environment');
      expect(getNodeAPIBaseUrl()).toBe('https://node.wread.com/api');
    });
