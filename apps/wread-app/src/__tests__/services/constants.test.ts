...
import {
  ...
  WREAD_NODE_BASE_URL,
  WREAD_UPDATER_FILE,
  WREAD_CHANGELOG_FILE,
  WREAD_PUBLIC_STORAGE_BASE_URL,
  WREAD_OPDS_USER_AGENT,
...
describe('services/constants', () => {
  ...
  describe('URL constants', () => {
    ...
    it('DOWNLOAD_WREAD_URL is a valid URL', () => {
      expect(DOWNLOAD_WREAD_URL).toMatch(/^https:\/\//);
    });

    it('WREAD_WEB_BASE_URL is a valid URL', () => {
      expect(WREAD_WEB_BASE_URL).toMatch(/^https:\/\//);
    });

    it('WREAD_NODE_BASE_URL is a valid URL', () => {
      expect(WREAD_NODE_BASE_URL).toMatch(/^https:\/\//);
    });

    it('WREAD_UPDATER_FILE is a URL ending with .json', () => {
      expect(WREAD_UPDATER_FILE).toMatch(/^https:\/\//);
      expect(WREAD_UPDATER_FILE).toMatch(/\.json$/);
    });

    it('WREAD_CHANGELOG_FILE is a URL ending with .json', () => {
      expect(WREAD_CHANGELOG_FILE).toMatch(/^https:\/\//);
      expect(WREAD_CHANGELOG_FILE).toMatch(/\.json$/);
    });

    it('WREAD_PUBLIC_STORAGE_BASE_URL is a valid URL', () => {
      expect(WREAD_PUBLIC_STORAGE_BASE_URL).toMatch(/^https:\/\//);
    });

    it('WREAD_OPDS_USER_AGENT is a non-empty string', () => {
      expect(typeof WREAD_OPDS_USER_AGENT).toBe('string');
      expect(WREAD_OPDS_USER_AGENT.length).toBeGreaterThan(0);
    });
  });
