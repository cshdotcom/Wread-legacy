...
const settings: WebDAVSettings = {
  ...
  // below can hard-code "/WRead/books/<hash>".
  rootPath: '/',
};
...
describe('deleteDirectory', () => {
  test('204 No Content resolves without throwing', async () => {
    ...
    await expect(deleteDirectory(config, '/WRead/books/abc')).resolves.toBeUndefined();
  });
  ...
  test('404 Not Found is treated as success (already gone)', async () => {
    ...
    await expect(deleteDirectory(config, '/WRead/books/missing')).resolves.toBeUndefined();
  });

  test('401 Unauthorized throws AUTH_FAILED', async () => {
    fetchMock.mockResolvedValueOnce(buildResponse(401));
    await expect(deleteDirectory(config, '/WRead/books/abc')).rejects.toMatchObject({
      code: 'AUTH_FAILED',
      status: 401,
    });
  });
  ...
  test('sends DELETE with explicit Depth: infinity header', async () => {
    ...
    fetchMock.mockResolvedValueOnce(buildResponse(204));
    await deleteDirectory(config, '/WRead/books/abc');
    expect(fetchMock).toHaveBeenCalledTimes(1);
    const [url, init] = fetchMock.mock.calls[0]!;
    expect(url).toBe('https://dav.example.com/WRead/books/abc');
...
describe('deleteRemoteBookDir', () => {
  ...
  test('targets the correct per-hash directory under <rootPath>/WRead/books', async () => {
    ...
    expect(url).toBe('https://dav.example.com/MyDav/WRead/books/abc123');
  });
});
