...
declare global {
  interface Window {
    __WREAD_IS_EINK?: boolean;
    __WREAD_IS_APPIMAGE?: boolean;
    __WREAD_UPDATER_DISABLED?: boolean;
  }
}
...
const DIST_CHANNEL = (process.env['NEXT_PUBLIC_DIST_CHANNEL'] || 'wread') as DistChannel;
...
export class NativeAppService extends BaseAppService {
  ...
  override isAppImage = Boolean(window.__WREAD_IS_APPIMAGE);
  override isEink = Boolean(window.__WREAD_IS_EINK);
  ...
  override hasUpdater =
    ...
    !window.__WREAD_UPDATER_DISABLED;
  ...
  async migrate20251029() {
    ...
    const oldDir = await join(rootPath, 'Images', 'WRead', 'Images');
    const dirToDelete = await join(rootPath, 'Images', 'WRead');
    ...
    if (!(await this.fs.exists(oldDir, 'None'))) {
      console.log('Migration 20251029: legacy Images/WRead/Images not found, skipping.');
      return;
    }
