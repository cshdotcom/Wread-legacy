...
import { isWReadBuiltinDict, WREAD_BUILTIN_DICTIONARIES, getBuiltinDictUrl } from './builtinDicts';
...
const getOrCreate = (
  ...
  // WRead built-in MDX dictionaries: these have wread-builtin- prefix IDs
  // and need a special file opener that fetches from /dictionaries/ URL
  if (isWReadBuiltinDict(id)) {
    const builtinDef = WREAD_BUILTIN_DICTIONARIES.find((d) => d.id === id);
    if (!builtinDef) return undefined;
    const builtinDict: ImportedDictionary = {
      id: builtinDef.id,
      kind: 'mdict',
      name: builtinDef.name,
      bundleDir: `wread-builtin/${builtinDef.id}`,
      files: { mdx: builtinDef.filename },
      lang: builtinDef.lang,
      addedAt: 0,
      unavailable: false,
      unsupported: false,
    };
    // Create a web-based file opener for built-in dictionaries
    const builtinFs: DictionaryFileOpener = {
      open: async (relativePath: string) => {
        // relativePath is the filename within the bundleDir
        const url = getBuiltinDictUrl(relativePath);
        const response = await fetch(url);
        if (!response.ok) throw new Error(`Failed to load built-in dictionary: ${relativePath}`);
        const blob = await response.blob();
        return blob;
      },
    };
    const provider = createMdictProvider({ dict: builtinDict, fs: builtinFs });
    instanceCache.set(id, provider);
    return provider;
  }
...
export const getEnabledProviders = ({
  ...
  for (const id of settings.providerOrder) {
    ...
    // WRead built-in dictionaries: handle like imported dictionaries
    if (isWReadBuiltinDict(id)) {
