...
const cleanVersion = (v: string): string => (v.startsWith('v') ? v.slice(1) : v);
...
export const checkForAppUpdates = async (
  ...
  // Tauri desktop platforms use the built-in updater
  if (isTauriAppPlatform()) {
    ...
    if (['macos', 'windows', 'linux'].includes(OS_TYPE)) {
      ...
      try {
        const response = await fetch(WREAD_UPDATER_FILE, { connectTimeout: 5000 });
        const data = await response.json();
        // Gitea API returns tag_name instead of version
        const latestVersion = cleanVersion(data.tag_name || data.version || '');
        const isNewer = latestVersion && semver.gt(latestVersion, getAppVersion());
        if (isNewer) {
          setUpdaterWindowVisible(true, latestVersion, getAppVersion());
        }
        return !!isNewer;
      } catch (err) {
        console.warn('Failed to fetch update info from Gitea', err);
        throw new Error('Failed to fetch update info');
      }
    }
  } else {
    // Web platform — check Gitea API for latest release
    try {
      const res = await window.fetch(WREAD_UPDATER_FILE);
      if (res.ok) {
...
export const checkAppReleaseNotes = async (isAutoCheck = true) => {
  ...
  if ((lastShownVersion && semver.gt(currentVersion, lastShownVersion)) || !isAutoCheck) {
    try {
      ...
      const res = await fetchFunc(WREAD_CHANGELOG_FILE);
      if (res.ok) {
