...
/**
 * Handle "Share to WRead" article URLs from the OS share sheet
 ...
 *      NSUserDefaults at `group.com.bilingify.wread`, and the host
 *      plugin (NativeBridgePlugin) drains them on foreground by calling
 *      `window.__wreadOnShareExtensionPending(saves)`. Same ingest
 *      pipeline, but the chosen library group is preserved.
 *
 * On iOS we also expose `window.__wreadGetGroups()` so the
 ...
 *   - http(s) (not file://, content://, wread://, blob:, data:)
...
export function useClipUrlIngress() {
  ...
  useEffect(() => {
    ...
    const handle = (url: string) => {
      ...
      //       https://web.wread.com/clip?url=<encoded>
      //   - Custom URL scheme (fallback):
      //       wread://clip?url=<encoded>
      const isClipUrl =
        url.startsWith('wread://clip?') ||
        url.startsWith('wread://clip/') ||
        /^https:\/\/web\.wread\.com\/clip(?:[/?].*)?$/i.test(url);
      if (isClipUrl) {
        try {
      ...
      // Annotation deep links can come over https (web.wread.com).
  ...
  useEffect(() => {
    ...
    const w = window as unknown as {
      ...
      __wreadOnShareExtensionPending?: (saves: PendingShareSave[]) => boolean;
      webkit?: {
        messageHandlers?: {
          wreadShareBridge?: { postMessage: (msg: { type: string }) => void };
        };
      };
    };
    ...
    w.__wreadOnShareExtensionPending = (saves) => {
    ...
    try {
      w.webkit?.messageHandlers?.wreadShareBridge?.postMessage({ type: 'ready' });
    } catch {
    ...
    return () => {
      const cleanup = window as unknown as {
        __wreadGetGroups?: unknown;
        __wreadOnShareExtensionPending?: unknown;
      };
      delete cleanup.__wreadGetGroups;
      delete cleanup.__wreadOnShareExtensionPending;
    };
  }, [appService, clipAndImport]);
}
