...
/**
 * Handle "Open with WRead" file imports. Consumes the `app-incoming-url`
 ...
 *
 * Non-file URL shapes (https, wread://, data:, blob:) are skipped here
 ...
 *
 *   `'VIEW'` (Android only — user picked WRead from the system "Open with"
 ...
 *   WRead" share is meant to honour.
 */
  ...
  useEffect(() => {
    ...
    const normalizeUrls = (urls: string[]): string[] => {
      ...
      for (let url of urls) {
        ...
        if (!/^(https?:|data:|blob:|wread:)/i.test(url)) {
          filePaths.push(url);
        }
      }
      return filePaths;
