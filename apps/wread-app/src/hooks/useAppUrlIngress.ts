...
interface SharedIntentPayload {
  urls: string[];
  /**
   * Android-only. Distinguishes "Open with WRead" (`VIEW` — the user
   * tapped a file in their file browser and chose WRead) from "Send to
   * WRead" (`SEND` / `SEND_MULTIPLE` — share-sheet capture). We forward
 ...
 *   - `shared-intent` plugin   — Android "Share to WRead" intent
...
export function useAppUrlIngress() {
  ...
  useEffect(() => {
    ...
    // side, so any "Open with WRead" intent that arrived AFTER cold
