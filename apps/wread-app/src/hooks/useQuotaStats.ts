...
export const useQuotaStats = (briefName = false) => {
  ...
  useEffect(() => {
    ...
    const isUnlimited = storagPlan.isUnlimited;

    // For unlimited users, show different display
    const storageQuota: QuotaType = isUnlimited
      ? {
          name: briefName ? _('Storage') : _('Cloud Sync Storage'),
          tooltip: _('Unlimited storage (Admin)'),
          used: parseFloat((storagPlan.usage / 1024 / 1024 / 1024).toFixed(2)),
          total: 0, // 0 = unlimited
          unit: 'GB',
        }
      : (() => {
