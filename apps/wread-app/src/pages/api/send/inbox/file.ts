...
function decodeRfc5987(value: string): string {
  // `X-WRead-Title: UTF-8''Spa%C3%9F`. Used so non-ASCII titles survive
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const titleRaw = header(req, 'x-wread-title');
  const urlRaw = header(req, 'x-wread-url');
  ...
  if ((count ?? 0) >= SEND_INBOX_PENDING_LIMIT) {
    return res.status(429).json({ error: 'Inbox is full — open WRead to process pending items' });
  }
