...
import { WREAD_WEB_BASE_URL, SHARE_BASE_URL, SHARE_TOKEN_LENGTH } from '@/services/constants';
...
export const parseShareDeepLink = (url: string): ShareDeepLink | null => {
  ...
  if (parsed.protocol === 'wread:') {
    // For wread://share/{token} the host portion holds the path segment
  ...
  if (parsed.protocol === 'https:' || parsed.protocol === 'http:') {
    if (!isWebWReadHost(parsed.host)) return null;
...
const isWebWReadHost = (host: string): boolean => {
  // Matches the production host and any preview domain WRead may serve from.
  // Conservative: accepts only the exact production host or a *.wread.com
  // subdomain so a third-party site cannot impersonate a share URL.
  if (host === new URL(WREAD_WEB_BASE_URL).host) return true;
  return host.endsWith('.wread.com');
};
