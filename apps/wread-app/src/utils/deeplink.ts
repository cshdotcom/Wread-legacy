import { WREAD_WEB_BASE_URL } from '@/services/constants';
 ...
 * `wread://` app deeplink or the universal `https://` web link.
 */
 ...
 * and Readwise sync. Mobile App Links (web.wread.com) intercept this URL
...
export const buildAnnotationWebUrl = ({ bookHash, noteId, cfi }: AnnotationDeepLink): string => {
  const base = `${WREAD_WEB_BASE_URL}${ANNOTATION_PATH_PREFIX}${bookHash}/annotation/${noteId}`;
...
export const buildAnnotationAppUrl = ({ bookHash, noteId, cfi }: AnnotationDeepLink): string => {
  const base = `wread://book/${bookHash}/annotation/${noteId}`;
  return cfi ? `${base}?cfi=${encodeURIComponent(cfi)}` : base;
};
...
/**
 * Parse an incoming wread:// or https://web.wread.com annotation URL.
...
export const parseAnnotationDeepLink = (url: string): AnnotationDeepLink | null => {
  ...
  const isCustomScheme = parsed.protocol === 'wread:';
  const isWebHost =
    (parsed.protocol === 'https:' || parsed.protocol === 'http:') &&
    parsed.host === 'web.wread.com';
  if (!isCustomScheme && !isWebHost) return null;

  // For wread:// URLs the URL parser stores the first path segment in the
