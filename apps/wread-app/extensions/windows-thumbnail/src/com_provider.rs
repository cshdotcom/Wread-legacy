/// Windows COM Thumbnail Provider for WRead
///
...
///
/// **Important**: Thumbnails are only shown when WRead.exe is the default application
/// for the file type.
///
...
/// CLSID: {A1B2C3D4-E5F6-7890-ABCD-EF1234567890}
pub const CLSID_WREAD_THUMBNAIL: GUID = GUID::from_u128(0xA1B2C3D4_E5F6_7890_ABCD_EF1234567890);
...
/// Check if WRead.exe is the default application for a given file extension.
fn is_wread_default_for_extension(ext: &str) -> bool {
...
/// Check if WRead is the default app for a specific file path.
fn is_wread_default_for_file(path: &PathBuf) -> bool {
    if let Some(ext) = path.extension().and_then(|e| e.to_str()) {
        let ext_with_dot = format!(".{}", ext.to_lowercase());
        return is_wread_default_for_extension(&ext_with_dot);
    }
    false
}
...
impl IInitializeWithItem_Impl for ThumbnailProvider_Impl {
    fn Initialize(&self, psi: Ref<'_, IShellItem>, _grfmode: u32) -> windows::core::Result<()> {
        ...
        unsafe {
            ...
            let is_default = is_wread_default_for_file(&path);
...
pub unsafe extern "system" fn DllGetClassObject(
    ...
    *ppv = std::ptr::null_mut();
    ...
    if *rclsid != CLSID_WREAD_THUMBNAIL {
...
fn clsid_string() -> String {
    format!(
        ...
        CLSID_WREAD_THUMBNAIL.data1,
        CLSID_WREAD_THUMBNAIL.data2,
        CLSID_WREAD_THUMBNAIL.data3,
        CLSID_WREAD_THUMBNAIL.data4[0],
        CLSID_WREAD_THUMBNAIL.data4[1],
        CLSID_WREAD_THUMBNAIL.data4[2],
        CLSID_WREAD_THUMBNAIL.data4[3],
        CLSID_WREAD_THUMBNAIL.data4[4],
        CLSID_WREAD_THUMBNAIL.data4[5],
        CLSID_WREAD_THUMBNAIL.data4[6],
        CLSID_WREAD_THUMBNAIL.data4[7]
    )
}
...
unsafe fn register_server_impl() -> Result<(), HRESULT> {
    ...
    set_reg_value(clsid_key, "", "WRead Thumbnail Provider")?;
