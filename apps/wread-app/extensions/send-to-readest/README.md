# Send to WRead — browser extension

One-click capture of the current web page into your WRead library as a
...
WRead client to open drains the inbox and imports the EPUB as-is — no
...
[D5 in the Send to WRead plan](../../docs/) and Part 4a / Part 8 of the
...
server (src/pages/api/send/inbox/file.ts)
   ...
   ▼
next WRead open → drainer imports the EPUB → book in library on all devices
```

A second always-on content script (`content/auth-bridge.ts`) runs only on
`web.wread.com` and copies the user's Supabase access token into the
...
3. **Load unpacked** → select this directory's `dist/` folder (not the
   project root).
4. Visit <https://web.wread.com> once and sign in so the auth-bridge
...
### Pointing the extension at a local WRead

The extension reads `chrome.storage.local.wreadApiBase` if set, falling
back to `https://web.wread.com`. From the DevTools console of the
...
Vitest exercises the extension's shell — upload (`X-WRead-*` headers, RFC
...
phase. From the `apps/wread-app` workspace root:
...
`apps/wread-app/src/__tests__/services/`. The unification regression
...
The extension uses **key-as-content** i18n (matches the wread-app's
...
`apps/wread-app/i18n-langs.json`. The extension's extractor reads the
...
1. Reads the canonical locale list from
   `apps/wread-app/i18n-langs.json` and ensures a stub
...
4. For every non-`en` locale: adds missing entries with
   `__STRING_NOT_TRANSLATED__` (same sentinel wread-app uses), keeps
...
1. Add the code to `apps/wread-app/i18n-langs.json`.
...
- Replace the icon set (currently a 1:1 downscale of the WRead app icon)
