...
const LOGIN_URL = 'https://web.wread.com/';
...
const openWReadBtn = $<HTMLButtonElement>('open-wread');
...
function render(progress: ClipProgress | null): void {
  switch (progress?.phase) {
    ...
    case 'uploading':
      sendBtn.disabled = true;
      showProgress(_('Sending to WRead…'));
      setStatus('');
      break;
    case 'done':
...
sendBtn.addEventListener('click', async () => {
  ...
  const request: ClipRequest = { type: 'send-to-wread:clip', tabId: currentTabId };
  await chrome.runtime.sendMessage(request).catch(() => undefined);
});

openWReadBtn.addEventListener('click', () => {
...
chrome.runtime.onMessage.addListener((message: unknown): void => {
  ...
  if (m.type === 'send-to-wread:progress' && m.progress) {
    render(m.progress);
  }
});
