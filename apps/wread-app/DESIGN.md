## WRead Design Language

WRead's UI is **Adwaita-aligned**, **e-ink-first**, **cross-platform-aware**. This doc is the
...
WRead's visual language descends from **Adwaita / libadwaita** — GNOME's design system —
...
- **Cross-platform reality.** WRead runs on macOS, Windows, Linux, iOS, Android, web. The
...
WRead ships with RTL languages enabled. **Never use direction-bound Tailwind
...
<div className='w-full'>
  ...
  <p className='text-base-content/70 text-sm leading-relaxed'>
    {_('Connect WRead to external services for sync, highlights, and catalogs.')}
  </p>
</div>
```
...
- **Card / View**: `rounded-lg` (8px) — WRead's house radius. Adwaita uses 9px; 8px is
...
Delete, remove, irreversible. Adwaita uses `destructive-action`. WRead uses red
...
A WRead-named archetype for "add another row to the list above" affordances. The two
...
These names come from libadwaita and apply 1:1 to WRead's lists. Use the names in code
...
`snapHeight` prop. Adwaita doesn't have a native sheet but WRead's mobile pattern is
the closest analog.
...
WRead ships on **macOS, Windows, Linux, iOS, Android, web**. Adwaita is desktop-GNOME-
...
- Material 3 conventions that conflict with Adwaita (FABs, elevation shadows, ripple
  inks): **don't** copy them. WRead's identity is Adwaita; the user is reading on
...
- Native Adwaita territory. WRead can match host theme for window chrome (Tauri
...
- Title bar height matches platform convention; internal layout follows WRead's
  Adwaita palette.
...
Why: hard-coded colors don't theme. WRead has 11 themes plus user-defined custom themes.
...
- **Adwaita / libadwaita**: GNOME's design system and widget toolkit. Source of WRead's
  visual lineage.
- **AdwActionRow / AdwSwitchRow / AdwComboRow / AdwExpanderRow**: libadwaita's row
  primitives. WRead mirrors these conceptually with custom React components.
...
- **ListExtension**: WRead-named archetype for "+ add new row" buttons (§4.6).
