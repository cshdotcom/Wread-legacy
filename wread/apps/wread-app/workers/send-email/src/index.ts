...
// Extensions WRead reads natively or converts client-side after import.
...
export default {
  async email(message: ForwardableEmailMessage, env: Env): Promise<void> {
    ...
    if (!['plus', 'pro', 'purchase'].includes(userPlan)) {
      message.setReject(
        'Send-to-WRead email-in requires the Plus, Pro, or Lifetime plan. ' +
          'Open the WRead app to upgrade, or clip articles for free with the ' +
    ...
    if (!senderRow || senderRow.status !== 'approved') {
      ...
      message.setReject('Sender not approved — approve it in WRead settings');
      return;
    }
    ...
    if ((count ?? 0) >= limit) {
      message.setReject('Inbox is full — open WRead to process pending items');
      return;
    }
