...
`schemaVersion` identifies the raw `config.json` schema written by WRead.
Current version:
...
overrides and are merged with defaults when WRead loads the config.
...
For notes in `booknotes`, WRead stores note ranges with:
