...
This document gives a system-level view of WRead: how the pieces fit together,
...
WRead is a single TypeScript/React codebase (`apps/wread-app`) compiled into
multiple targets:
...
- a **web app** running on Next.js / Cloudflare Workers (OpenNext) at
  [web.wread.com](https://web.wread.com)
- two **side surfaces**: a "Send to WRead" browser extension
  (`apps/wread-app/extension/send-to-wread`) and a Windows thumbnail
  shell extension (`apps/wread-app/extensions/windows-thumbnail`)
...
flowchart LR
    subgraph Clients
        ...
        Ext["Browser extension<br/>(Send to WRead)"]
        WinExt["Windows shell ext<br/>(thumbnail provider)"]
    end

    subgraph Backend["WRead backend (Next.js routes + Cloudflare Worker)"]
...
flowchart TB
    ...
    Send["app/send<br/>(send-to-WRead inbox)"]
...
forked copy of the Foliate JS engine. WRead's reader code in `app/reader` and
...
send/inbox.ts            -> "Send to WRead" inbox listing
...
`apps/wread-app/workers/send-email` is a separate Cloudflare Worker
(deployed independently from the main app) responsible for the "Send to WRead
...
"Send to WRead" article-to-EPUB conversion.

### 6.11 Send to WRead

End-to-end pipeline:

1. The browser extension (`apps/wread-app/extension/send-to-wread`) or the
...
```mermaid
flowchart LR
    Source["apps/wread-app (single source)"]
    ...
    subgraph DeployTargets
        DCloudflare["Cloudflare Workers<br/>(web.wread.com)"]
        DDocker["Docker image<br/>(ghcr.io/wread/wread)"]
