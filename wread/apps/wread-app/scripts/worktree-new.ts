...
if (/^\d+$/.test(arg)) {
  ...
  localBranch = `pr-${arg}`;
  worktreePath = path.join(path.dirname(repoRoot), `wread-${localBranch}`);
  ...
  localBranch = arg;
  worktreePath = path.join(path.dirname(repoRoot), `wread-${arg.replace(/\//g, '-')}`);
...
if (fs.existsSync(androidGenDir)) {
  ...
  execSync('pnpm tauri icon ../../data/icons/wread-book.png', {
    stdio: gitStdio,
    cwd: dstAppDir,
  });
...
// src-tauri), per-user Xcode state, and `Pods/` (defensive; WRead's iOS
