...
//
//   The first attempt used a custom `wread-clip://` URI scheme + `fetch`.
...
/// are English / WRead's dark palette so a caller that omits a field
...
impl ClipOptions {
    fn window_title(&self) -> &str {
        self.window_title
            .as_deref()
            .unwrap_or("Saving to your WRead library…")
    }
    fn overlay_title(&self) -> &str {
        self.overlay_title.as_deref().unwrap_or("Saving to WRead")
    }
    ...
    fn saved_title(&self) -> &str {
        self.saved_title.as_deref().unwrap_or("Saved to WRead")
    }
...
fn loading_overlay_script(
    ...
    format!(
        ...
        (function() {{
          ...
          var FG = {fg_json};
          function install() {{
            if (document.getElementById('__wread_overlay__')) return;
            if (!document.documentElement) return;
            var ov = document.createElement('div');
            ov.id = '__wread_overlay__';
            ...
            spin.style.cssText = 'width:36px;height:36px;border:3px solid color-mix(in srgb,' +
              ...
              'border-radius:50%;animation:__wread_spin__ 0.8s linear infinite';
            var title = document.createElement('div');
            title.style.cssText = 'font-size:15px;font-weight:600';
            title.textContent = TITLE;
            var status = document.createElement('div');
            status.id = '__wread_status__';
            status.style.cssText = 'font-size:13px;opacity:0.7;max-width:340px;' +
              'overflow:hidden;text-overflow:ellipsis;white-space:nowrap';
            status.textContent = STATUS;
            var style = document.createElement('style');
            style.textContent = '@keyframes __wread_spin__{{to{{transform:rotate(360deg)}}}}';
            ov.appendChild(spin);
          ...
          window.__wread_setStatus__ = function(text) {{
            var el = document.getElementById('__wread_status__');
...
pub async fn clip_url(
    ...
    let init_script = format!(
        r#"
        (function() {{
          console.log('[wread-clip] init script running');
          var PORT = {port};
          ...
          function send(reason) {{
            ...
            sent = true;
            try {{
              if (window.__wread_setStatus__) {{
                window.__wread_setStatus__(CAPTURING_STATUS);
              }}
              var html = document.documentElement.outerHTML;
              console.log('[wread-clip] capturing reason=' + reason +
                ' bytes=' + html.length);
              ...
              console.warn('[wread-clip] navigate threw:', e && e.message);
            }}
          }}
    ...
    // chrome style WRead's main/reader windows use so it doesn't read
