...
describe('ingestFile', () => {
  ...
  test('does not mark in-place for files inside wread-managed Books prefix', async () => {
    // Files copied under wread's own AppData Books/<hash>/ are already
    // hash copies, not user originals. Marking them in-place would set
    // book.filePath to a path wread already controls.
    const { appService, settings, isLoggedIn, importBook } = makeDeps({
  ...
  test('matches across multiple roots; user-owned Books/ subdirs go in-place', async () => {
    ...
    // content — files there go in-place. Only wread's own managed Books
