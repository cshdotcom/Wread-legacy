...
describe('buildRequestUrl (encodePath)', () => {
  test('escapes spaces and unicode in each segment', () => {
    expect(buildRequestUrl('https://dav.example.com', '/WRead/My Books/小说.epub')).toBe(
      'https://dav.example.com/WRead/My%20Books/%E5%B0%8F%E8%AF%B4.epub',
    );
  });
  ...
  test("preserves existing %-escapes — the comment's promise that we don't double-encode", () => {
    ...
    expect(buildRequestUrl('https://dav.example.com', '/WRead/My%20Books/file.epub')).toBe(
      'https://dav.example.com/WRead/My%20Books/file.epub',
    );
  });
