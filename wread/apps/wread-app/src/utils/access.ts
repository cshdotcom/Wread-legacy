...
import { validateUserAndToken as wreadValidateUserAndToken } from '@/utils/wread-auth';
...
/**
 * Validate user and token from Authorization header.
 * In WRead, this uses local JWT verification instead of Supabase.
 * The return format is compatible with the original function.
 */
export const validateUserAndToken = async (authHeader: string | null | undefined) => {
  return wreadValidateUserAndToken(authHeader);
};
