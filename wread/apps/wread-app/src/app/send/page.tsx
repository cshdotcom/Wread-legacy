...
/**
 * The Send to WRead web page. Being a WRead client itself, it runs the
...
export default function SendPage() {
  ...
  if (!user) {
    return (
      <div className='mx-auto flex max-w-[560px] flex-col items-center px-4 py-16 text-center'>
        <h1 className='text-xl font-semibold'>{_('Send to WRead')}</h1>
  ...
  return (
    <div className='mx-auto flex max-w-[560px] flex-col gap-6 px-4 py-10'>
      <header>
        <h1 className='text-xl font-semibold tracking-tight'>{_('Send to WRead')}</h1>
      ...
      {canClipFromUrl ? (
        ...
        <section className='card eink-bordered border-base-200 bg-base-100 flex flex-col gap-2 border p-5'>
          ...
          <p className='text-base-content/70 text-xs leading-relaxed'>
            {_(
              'Install the WRead browser extension to send the article you are reading to your library. It clips the page from your browser so paywalled and login-only sites still work.',
            )}
          </p>
        </section>
      )}
