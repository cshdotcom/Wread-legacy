...
interface SendToWReadFormProps {
  onBack: () => void;
}
...
const SendToWReadForm: React.FC<SendToWReadFormProps> = ({ onBack }) => {
  ...
  const load = useCallback(async () => {
    try {
      ...
      toast(_('Could not load Send to WRead settings'), 'error');
    } finally {
      setLoading(false);
    }
        ...
        <div className='card eink-bordered border-base-200 bg-base-100 overflow-hidden border'>
          <div className='flex flex-col items-center gap-3 px-6 py-8 text-center'>
            ...
            <p className='text-base-content/70 max-w-sm text-sm leading-relaxed'>
              {_(
                'Forward attachments and articles to your private WRead address. Available on the Plus, Pro, and Lifetime plans.',
              )}
            </p>
            <button
              type='button'
...
export default SendToWReadForm;
