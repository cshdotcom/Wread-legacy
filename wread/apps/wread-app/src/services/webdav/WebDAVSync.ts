 ...
 *   <rootPath>/WRead/books/<hash>/config.json
 *
 * The wire format is a thin envelope around the existing local
 * `BookConfig` so the merge logic can stay identical to wread's other
 ...
 *   - wread's own cloud sync similarly carves out viewSettings from
...
/**
 * Pull `<rootPath>/WRead/books/<hash>/config.json`, merge into the
 ...
 *   <rootPath>/WRead/books/<hash>/<safe-title>.<ext>
 *
 ...
 *   <rootPath>/WRead/books/<hash>/cover.png
 *
 ...
 *   - wread fetches custom covers via metadata services that produce
...
/**
 * Delete the per-book directory `<rootPath>/WRead/books/<hash>/`
 ...
 *      anything yet).
 *   2. `pushBookConfig` — creates `WRead/books/<hash>/config.json`.
