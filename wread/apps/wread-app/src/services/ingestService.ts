...
export interface IngestFileDeps {
  ...
  isLoggedIn: boolean;
  /**
   * Pre-resolved absolute path to WRead's own `Books/` directory. When
...
export interface IngestFileOptions {
  ...
  groupId?: string;
  groupName?: string;
  /** Tag parsed from a Send-to-WRead email subject (`#scifi`). */
  subjectTag?: string;
 ...
 *     explicitly told WRead to read in place. The WRead data location
 *     (`customRootDir`) is intentionally NOT an in-place trigger; that
 *     directory is WRead's own home and may freely contain hash copies.
 *   - The path is NOT inside WRead's own managed books directory
 *     (`appBooksPrefix`, e.g. `<AppData>/Books/`). Anything in that subtree
 *     is a hash copy under WRead's control, no point marking it in-place.
...
function shouldImportInPlace(
  ...
  // If the file already lives inside WRead's own managed books directory
