...
const POPUP_DOM = `
  ...
  <section id="signed-in-view">
    ...
    <button id="send" class="primary" disabled>Send to WRead</button>
  ...
  <section id="signed-out-view" class="sign-in hidden">
    <p>Sign in to WRead to start clipping pages.</p>
    <button id="open-wread" class="primary">Open web.wread.com</button>
  </section>
`;
...
function setStatus(overrides: Partial<StatusResponse> = {}): void {
  chromeMock.runtime.sendMessage.mockImplementation(async (msg: unknown) => {
    if (
      msg &&
      typeof msg === 'object' &&
      (msg as { type?: string }).type === 'send-to-wread:status'
    ) {
      return {
        signedIn: true,
        inFlight: false,
...
function pushProgress(progress: ClipProgress): void {
  ...
  progressHandler({ type: 'send-to-wread:progress', progress });
}
...
describe('popup — render(progress)', () => {
  ...
  test('"uploading" shows the send-to-WRead label', async () => {
    await loadPopup();
    pushProgress({ phase: 'uploading' });
    expect(document.getElementById('progress-label')?.textContent).toBe('Sending to WRead…');
  });
...
describe('popup — Send button', () => {
  test('clicking Send dispatches a clip request with the active tabId', async () => {
    ...
    const clipCall = chromeMock.runtime.sendMessage.mock.calls.find((c) => {
      ...
      return m?.type === 'send-to-wread:clip';
    });
    expect(clipCall).toBeDefined();
    expect(clipCall![0]).toEqual({ type: 'send-to-wread:clip', tabId: 7 });
  });
});
