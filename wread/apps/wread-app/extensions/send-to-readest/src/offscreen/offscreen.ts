 ...
 *
 * Protocol:
 *   SW → offscreen: { type:'send-to-wread:clip-and-upload',
 *                     html, url, token }
...
const LOG = '[send-to-wread/offscreen]';
...
chrome.runtime.onMessage.addListener((message: unknown, _sender, sendResponse): boolean => {
  ...
  if (m.type !== 'send-to-wread:ping') return false;
  sendResponse({ ok: true });
  return false;
});

interface ClipAndUploadRequest {
  type: 'send-to-wread:clip-and-upload';
  html: string;
  url: string;
  token: string;
  pageTitle: string;
  /** Resolved upload URL — the SW reads `chrome.storage.local.wreadApiBase`
...
chrome.runtime.onMessage.addListener(
  (message: unknown, _sender, sendResponse: (response: ClipAndUploadResponse) => void): boolean => {
    if (!message || typeof message !== 'object') return false;
    const m = message as { type?: string };
    if (m.type !== 'send-to-wread:clip-and-upload') return false;
