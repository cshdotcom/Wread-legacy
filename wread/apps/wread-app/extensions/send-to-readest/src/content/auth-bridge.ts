/**
 * Runs on `web.wread.com` (and the dev host). Reads WRead's Supabase
...
function syncToken(): void {
  ...
  if (token) {
    chrome.storage.local.set({
      wreadAccessToken: token,
      wreadTokenAt: Date.now(),
    });
  } else {
    // Token went away (sign-out) — clear so the extension stops showing a
    // stale "signed in" state.
    chrome.storage.local.remove(['wreadAccessToken', 'wreadTokenAt']);
  }
}
...
// Still listen for cross-tab updates so a second WRead tab can refresh this
