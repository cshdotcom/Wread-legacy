/**
 * Send to WRead — background service worker. Orchestrates the clip:
 *
 *   popup → SW: `send-to-wread:clip` (with tabId)
 ...
 *   SW → popup: `send-to-wread:progress` broadcasts at each phase.
 */
...
function broadcast(progress: ClipProgress): void {
  ...
  chrome.runtime.sendMessage({ type: 'send-to-wread:progress', progress }).catch(() => undefined);
}
...
const CAPTURE_PORT_NAME = 'send-to-wread:capture';
...
async function runClip(tabId: number): Promise<void> {
  ...
  try {
    ...
    if (!stored) {
      return emitError('not-signed-in', _('Sign in at web.wread.com first'));
    }
...
chrome.runtime.onMessage.addListener((message: unknown, _sender, sendResponse): boolean => {
  ...
  if (m.type === 'send-to-wread:status') {
