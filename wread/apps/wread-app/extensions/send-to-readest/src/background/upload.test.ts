...
const DEFAULT_ENDPOINT = 'https://web.wread.com/api/send/inbox/file';
...
describe('resolveUploadEndpoint', () => {
  test('defaults to the production endpoint when no override is set', async () => {
    expect(await resolveUploadEndpoint()).toBe('https://web.wread.com/api/send/inbox/file');
  });

  test('honours wreadApiBase in chrome.storage.local', async () => {
    await chromeMock.storage.local.set({ wreadApiBase: 'http://localhost:3000' });
    expect(await resolveUploadEndpoint()).toBe('http://localhost:3000/api/send/inbox/file');
  });

  test('strips a trailing slash from wreadApiBase', async () => {
    await chromeMock.storage.local.set({ wreadApiBase: 'http://localhost:3000/' });
    expect(await resolveUploadEndpoint()).toBe('http://localhost:3000/api/send/inbox/file');
  });

  test('ignores wreadApiBase when it is not http(s)', async () => {
    await chromeMock.storage.local.set({ wreadApiBase: 'javascript:alert(1)' });
    expect(await resolveUploadEndpoint()).toBe('https://web.wread.com/api/send/inbox/file');
  });
...
describe('uploadEpub', () => {
  test('POSTs the EPUB body with Authorization + RFC 5987 headers', async () => {
    ...
    expect(url).toBe('https://web.wread.com/api/send/inbox/file');
    ...
    // header round-trip.
    expect(headers['X-WRead-Title']).toBe("UTF-8''Hello%2C%20World");
    expect(headers['X-WRead-Url']).toBe("UTF-8''https%3A%2F%2Fexample.com%2Farticle");
  ...
  test('encodes non-ASCII titles correctly', async () => {
    ...
    expect(headers['X-WRead-Title']).toBe(
  ...
  test('maps network failures to network-error and names the target host', async () => {
    ...
    if (!result.ok) {
      ...
      expect(result.message).toContain('web.wread.com');
