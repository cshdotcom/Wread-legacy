...
| --------------- | --------------------------- | ------------------------------------------------- |
| **client**      | `ghcr.io/wread/wread`   | wread frontend                                  |
...
this pulls `${READEST_IMAGE}` (default: `ghcr.io/wread/wread:latest`) instead of building the client locally.
...
```env
READEST_IMAGE=docker.io/your-dockerhub-username/wread:latest
```

replace `your-dockerhub-username` with the Docker Hub namespace that publishes your `wread` image.
